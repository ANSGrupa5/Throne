...
public sealed class MatchRules : ScriptableObject
{
    ...
    private void OnValidate()
    {
        ...
        MaxMatchDurationSeconds = Mathf.Max(MinMatchDurationSeconds, MaxMatchDurationSeconds);
        MaxTrailLength = Mathf.Max(MinTrailLength, MaxTrailLength);
        MaxRespawnTime = Mathf.Max(MinRespawnTime, MaxRespawnTime);
        MaxVehicleSpeed = Mathf.Max(MinVehicleSpeed, MaxVehicleSpeed);
    }
}

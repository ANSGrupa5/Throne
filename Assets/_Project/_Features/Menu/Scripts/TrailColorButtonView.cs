...
public sealed class TrailColorButtonView
{
    ...
    {
        if (button == null)
            Debug.LogError($"{nameof(MatchLobbyController)} is missing trail color button reference at index {index}.", owner);
        if (selectionFrame == null)
            Debug.LogError($"{nameof(MatchLobbyController)} is missing trail color selection frame at index {index}.", owner);
    }
}
